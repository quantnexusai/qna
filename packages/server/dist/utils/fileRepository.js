                    ...
                    if (input.startsWith('[')) {
                        try {
                            ...
                            for (let j = 0; j < files.length; j++) {
                                const file = files[j];
                                if (re.test(file)) {
                                    node.data.inputs[key] = await (0, flowise_components_1.addBase64FilesToStorage)(file, chatflowid, fileNames);
                                }
                            }
                        }
                        catch (e) {
                        ...
                        node.data.inputs[key] = await (0, flowise_components_1.addBase64FilesToStorage)(input, chatflowid, fileNames);
                    }
                }
            }
        }
