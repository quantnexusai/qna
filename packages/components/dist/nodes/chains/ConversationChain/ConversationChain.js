...
const prepareChain = async (nodeData, options, sessionId) => {
    let model = nodeData.inputs?.model;
    const memory = nodeData.inputs?.memory;
    ...
    if ((0, multiModalUtils_1.llmSupportsVision)(model)) {
        messageContent = await (0, multiModalUtils_1.addImagesToMessages)(nodeData, options, model.multiModalOption);
