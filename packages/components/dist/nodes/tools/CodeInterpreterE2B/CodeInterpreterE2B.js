...
class E2BTool extends tools_1.StructuredTool {
    ...
    async _call(arg, _, flowConfig) {
        ...
        try {
            if ('input' in arg) {
                this.instance = await code_interpreter_1.CodeInterpreter.create({ apiKey: this.apiKey });
                ...
                for (const result of execution.results) {
                    for (const key in result) {
                        ...
                        if (key === 'png') {
                            ...
                            const res = await (0, storageUtils_1.addSingleFileToStorage)('image/png', pngData, filename, this.chatflowid, flowConfig.chatId);
                            ...
                            const res = await (0, storageUtils_1.addSingleFileToStorage)('image/jpg', jpegData, filename, this.chatflowid, flowConfig.chatId);
