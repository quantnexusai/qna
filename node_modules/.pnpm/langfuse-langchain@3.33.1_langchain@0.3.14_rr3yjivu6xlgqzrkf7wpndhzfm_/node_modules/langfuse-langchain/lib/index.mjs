...
import { BaseMessage, AIMessage, AIMessageChunk, HumanMessage, ChatMessage, SystemMessage, FunctionMessage, ToolMessage } from '@langchain/core/messages';
...
class CallbackHandler extends BaseCallbackHandler {
    ...
    try {
      ...
      if (!parentRunId) {
        this.traceUpdates.set(runId, {
          tags,
          userId: metadata && "langfuseUserId" in metadata && typeof metadata["langfuseUserId"] === "string" ? metadata["langfuseUserId"] : undefined,
          sessionId: metadata && "langfuseSessionId" in metadata && typeof metadata["langfuseSessionId"] === "string" ? metadata["langfuseSessionId"] : undefined
        });
      }
    } catch (e) {
      this._log(e);
  ...
  extractUsageMetadata(generation) {
    try {
      const usageMetadata = "message" in generation && (generation["message"] instanceof AIMessage || generation["message"] instanceof AIMessageChunk) ? generation["message"].usage_metadata : undefined;
...
const createDatasetItemHandler = async params => {
  const {
    runName,
    item,
    langfuseClient,
...
export { CallbackHandler, createDatasetItemHandler, CallbackHandler as default };
