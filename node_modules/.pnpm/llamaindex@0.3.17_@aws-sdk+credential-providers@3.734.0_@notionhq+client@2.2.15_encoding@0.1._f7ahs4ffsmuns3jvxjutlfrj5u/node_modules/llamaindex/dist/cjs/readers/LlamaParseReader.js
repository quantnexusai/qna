...
class LlamaParseReader extends _type.FileReader {
   ...
   */ async getImages(jsonResult, downloadPath) {
        ...
        if (!_env.fs.access(downloadPath)) {
    ...
    async getMimeType(data) {
        ...
        const validMime = mimes.find((mime)=>Object.values(SupportedFiles).includes(mime));
        if (!validMime) {
            const supportedExtensions = Object.keys(SupportedFiles).join(", ");
            throw new Error(`File has type "${mimes}" which does not match supported MIME Types. Supported formats include: ${supportedExtensions}`);
        }
        return validMime;
    }
}
