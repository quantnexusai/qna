...
import { GEMINI_BACKENDS, GEMINI_MODEL } from "./types.js";
 ...
 */ export class GeminiSessionStore {
    ...
    static getSessionId(options) {
        if (options.backend === GEMINI_BACKENDS.GOOGLE) return options?.apiKey ?? "";
        return "";
    }
 ...
 */ export class Gemini extends ToolCallLLM {
    ...
    async complete(params) {
        ...
        const client = this.session.getGenerativeModel(this.metadata);
        if (stream) {
            const result = await client.generateContentStream(getPartsText(GeminiHelper.messageContentToGeminiParts(prompt)));
            return this.session.getCompletionStream(result);
        }
        const result = await client.generateContent(getPartsText(GeminiHelper.messageContentToGeminiParts(prompt)));
        return {
